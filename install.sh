...
if [ -n "${UV_DOWNLOAD_URL:-}" ]; then
    ...
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/astral-sh/uv/releases/download/0.8.17"
fi
...
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.28.7"},"source":{"app_name":"uv","name":"uv","owner":"astral-sh","release_type":"github"},"version":"0.8.17"}
EORECEIPT
...
The installer for uv 0.8.17

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/astral-sh/uv/releases/download/0.8.17
...
download_binary_and_run_installer() {
    ...
    case "$_artifact_name" in 
        ...
        "uv-i686-unknown-linux-musl.tar.gz")
            ...
            _updater_bin=""
            ;;
        "uv-loongarch64-unknown-linux-gnu.tar.gz")
            _arch="loongarch64-unknown-linux-gnu"
            _zip_ext=".tar.gz"
            _bins="uv uvx"
            _bins_js_array='"uv","uvx"'
            _libs=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        ...
        "i686-unknown-linux-musl-static")
            ...
            ;;
        "loongarch64-unknown-linux-gnu")
            _archive="uv-loongarch64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "36"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
                echo "$_archive"
